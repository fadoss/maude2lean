...
-- Definition of derived operators outside the inductive type

def kBool.or : kBool → kBool → kBool
	| a b := (kBool.xor (kBool.and a b) (kBool.xor a b))

def kBool.not : kBool → kBool
	| a := (kBool.xor kBool.true a)

def kNat.sum : kNat → kNat → kNat
	| n kNat.zero := n
	| n (kNat.s m) := (kNat.s (kNat.sum n m))

def kBool.implies : kBool → kBool → kBool
	| a b := (kBool.not (kBool.xor a (kBool.and a b)))
...
namespace kBool
	...
	attribute [congr] eqa.eqa_and eqe.eqe_and eqa.eqa_xor eqe.eqe_xor eqa_congr eqe_congr eqa_eqe_congr
	attribute [simp] has_sort.true_decl has_sort.false_decl has_sort.and_decl has_sort.xor_decl eqe.eq_and₀ eqe.eq_and₁ eqe.eq_and₂ eqe.eq_xor₀ eqe.eq_xor₁ eqe.eq_and₃ or not implies
end kBool
...
namespace kNat
	...
	attribute [symm] eqa.symm eqe.symm
	attribute [trans] eqa.trans eqe.trans
	attribute [congr] eqa.eqa_s eqe.eqe_s eqa_congr eqe_congr eqa_eqe_congr
	attribute [simp] has_sort.zero_decl has_sort.s_decl has_sort.mb_s sum subsort_nznat_nat subsort_even_nat
end kNat
